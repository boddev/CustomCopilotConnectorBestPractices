...
## 12. Enterprise Security & Production Readiness

Sections 1–11 focus on building a connector. This section addresses the security posture, credential management, deployment architecture, and compliance documentation that enterprise security teams expect **before granting admin consent** and promoting a connector to production.

### 12.1 Application vs. Delegated Permissions — Production Justification

Copilot Connectors operate as **daemon/service identities**, not end-user applications. They run on scheduled timers — typically a daily full crawl and incremental crawls every few hours — requiring unattended, headless execution. This architecture fundamentally requires application-level permissions.

#### Why Delegated Permissions Don't Work in Production

While Microsoft Graph APIs for External Connections and External Items technically list both delegated and application permissions as supported, delegated permissions introduce critical operational risks:

| Requirement | Delegated Permissions | Application Permissions |
|---|---|---|
| Unattended automation | ❌ Requires interactive user sign-in | ✅ Fully supported |
| Scheduled ingestion (daily + incremental) | ❌ Tokens expire with user sessions | ✅ Persistent, renewable tokens |
| Resilience to user lifecycle changes | ❌ Fails if user leaves org, is disabled, or MFA blocks token refresh | ✅ Independent of any user account |
...
> 📌 **Microsoft's own documentation states** that delegated permissions are supported for development and testing but are **not recommended for production ingestion scenarios**. Graph connectors represent a trusted system identity, not an individual user.

#### Justifying to Your Security Team

When preparing your admin consent request, document:
...
2. **Why `.OwnedBy` is sufficient** — The `.OwnedBy` variants (`ExternalConnection.ReadWrite.OwnedBy`, `ExternalItem.ReadWrite.OwnedBy`) restrict access to only the connections and items created by this specific app registration
3. **Why `.All` is not needed** — The `.All` variants would grant access to all tenant connections; `.OwnedBy` limits scope to this app's own connections — the minimum possible
...
### 12.2 Secret Management: Azure Key Vault & Managed Identity

Production connectors must **never store credentials in source code, configuration files, or environment variables**. Use Azure Key Vault with Managed Identity for a zero-credential deployment model.
...
2. **System-Assigned Managed Identity** — Enable on the Azure App Service (or compute host). This eliminates the need for storing any credentials in application code.

3. **Key Vault RBAC** — Assign the **Key Vault Secrets User** role to the App Service's Managed Identity. This grants `GET` permission on secrets only — nothing more.
...
> 💡 **Key insight**: Even when an OAuth scope is broad (e.g., Salesforce's `api` scope grants full REST API access), the **actual data access is controlled by the Integration User's Profile and Field-Level Security**, not by the OAuth scope alone. This is why layered controls are essential.

### 12.4 Unidirectional Data Flow / Read-Only Pipeline

Enterprise connectors should be designed as **one-directional, read-only pipelines**. Data flows from the source system into Microsoft Graph — never the reverse. This principle simplifies security review and limits the blast radius of any credential compromise.
...
#### Documenting API Operations

For your security review, create a complete **API operations inventory** for both the source system and Microsoft Graph. This table should enumerate every HTTP method, endpoint, purpose, and whether it writes data:
...
| `GET` | `/users/{id}?$select=id` | Map source user to Entra ID GUID (for ACLs) | Per unique user, cached |

> 📌 **Zero POST/PUT/PATCH/DELETE operations to source data objects.** This distinction is critical for security reviewers assessing whether the connector could inadvertently modify source data.

### 12.5 Defense in Depth — Layered Security Model

Enterprise connectors should implement **multiple independent security layers** so that no single control failure results in a breach. Even if one layer is bypassed (e.g., an OAuth scope is broad), subsequent layers restrict actual access.

#### Security Layers for a Production Connector

| Layer | Control | Effect | Notes |
|---|---|---|---|
| 1 | **OAuth Scope** | Grants API access to the source system | Often the broadest layer; may not be reducible further |
| 2 | **Dedicated Integration User** | Dedicated service account; no human login capability | Limits the identity to API-only access |
| 3 | **Read-Only Profile / Role** | Create/Edit/Delete disabled on all objects | Even if the OAuth scope permits writes, the profile blocks them |
| 4 | **Field-Level Security** | Blocks access to sensitive fields (passwords, PII, secrets) | Restricts which data fields are visible |
| 5 | **Connected App / OAuth Policy** | Admin-approved users only; bound to the read-only profile | Restricts who can use the OAuth credentials |
| 6 | **IP Allowlist + Rate Limits** | Restricts API calls to Azure outbound IPs; caps API volume | Network-level and volume-level controls |

#### Microsoft Graph Permission Layers

| Layer | Control | Effect |
|---|---|---|
| 1 | **`.OwnedBy` permission scope** | App can only access its own connections and items; cannot touch emails, files, calendars, Teams, OneDrive |
| 2 | **Azure Key Vault + Managed Identity** | No credentials in code; secrets accessible only to the authorized compute identity |
| 3 | **Key Vault RBAC** | Only the App Service Managed Identity has `Key Vault Secrets User` role (GET on secrets only) |
| 4 | **M365 Search ACL Trimming** | Ingested items include per-user ACLs; M365 Search enforces these at query time |

> 💡 **Why this matters**: A security reviewer can trace each layer and confirm that even if credentials are compromised, the combination of read-only profiles, IP restrictions, and scoped permissions limits the damage an attacker can do.

### 12.6 Blast Radius Analysis

A **blast radius analysis** documents the impact of credential compromise for each system the connector authenticates to. This is a standard enterprise security requirement and should be included in your admin consent justification package.

#### Template: Microsoft Entra App Credential Compromise

| Scenario | Risk Level | Impact | Mitigation |
|---|---|---|---|
| Attacker creates rogue connections | LOW | Connections visible in Admin Center; require schema + items to appear in search | Monitor via M365 Admin Center |
| Attacker injects fake search results | MEDIUM | Could insert items into search; ACLs still enforced at query time | Admin Center shows all items; Key Vault secret rotation |
| Attacker deletes the connection | MEDIUM | Indexed items lost; recoverable by re-running full crawl | Connection auto-recreated on next scheduled trigger |
| Attacker reads indexed items | LOW | Items contain only data already in the source system; no secrets stored | Data already accessible to authorized source users |
| Attacker accesses emails, files, calendars | **NOT POSSIBLE** | `.OwnedBy` scope prevents access to any M365 data outside this app's connections | Built-in Graph API scope boundary |
| Attacker accesses other apps' connections | **NOT POSSIBLE** | `.OwnedBy` limits access to connections created by this specific app | Built-in Graph API scope boundary |

#### Template: Source System Credential Compromise

| Scenario | Risk Level | Impact | Mitigation |
|---|---|---|---|
| Attacker reads data | HIGH (without profile restriction) | All objects the Integration User's profile can see | Read-Only Profile + FLS; restrict to only required objects |
| Attacker writes or modifies data | **NOT POSSIBLE** (with Read-Only Profile) | Even with broad OAuth scope, writes fail if Profile lacks Create/Edit permission | Read-Only Profile enforced at platform level |
| Attacker deletes records | **NOT POSSIBLE** (with Read-Only Profile) | Profile must explicitly grant Delete; Read-Only blocks this | Read-Only Profile enforced at platform level |
| Attacker exfiltrates data at scale | MEDIUM | Bulk queries possible within Profile's object scope | IP Allowlisting + API Rate Limits + Audit Logging |
| Attacker accesses other tenants/orgs | **NOT POSSIBLE** | Credentials are org-specific; cannot cross org boundaries | Platform-level boundary |

> 📌 **Adapt this template** to your specific source system. The key goal is to demonstrate that `.OwnedBy` scope + source system hardening limits the blast radius to the connector's own data, with no lateral movement possible.

### 12.7 Connector Deployment Architecture

Production connectors must run as reliable, unattended daemon workloads with scheduled execution. The recommended deployment model for Azure-hosted connectors:
...
|---|---|---|
| **Azure App Service** | Always-on daemon workloads with scheduled triggers | System-Assigned Managed Identity for Key Vault access |
| **Azure Functions (Timer Trigger)** | Lightweight, event-driven connectors | Consumption plan may have cold-start latency |
| **Azure Container Apps** | Complex connectors with specific runtime dependencies | Managed Identity supported; more operational overhead |
...
| **Azure VM** | Full control over runtime environment | Higher maintenance burden; use Managed Identity |

#### Crawl Scheduling Best Practices

| Crawl Type | Purpose | Recommended Frequency |
|---|---|---|
| **Full crawl** | Initial load + periodic reconciliation; ensures parity with source | Daily or weekly, depending on data volume |
| **Incremental crawl** | Detect additions, updates, and deletions since last checkpoint | Every 4–12 hours |
| **On-demand crawl** | Manual trigger after bulk source changes or schema updates | As needed |
...
Enable comprehensive audit logging on the source system to track all API calls made by the connector's integration user:

- **Salesforce**: Enable Setup Audit Trail and Event Monitoring. All API calls made by the Integration User are logged and reviewable.
- **ServiceNow**: Enable System Audit with table-level auditing for accessed records.
- **Custom APIs**: Implement request logging with correlation IDs that match your connector's crawl sessions.

#### Microsoft Graph Audit Logging

Monitor connector activity in the M365 Admin Center under **Search & intelligence > Connectors**:
...
Restrict source system API access to known Azure outbound IP ranges:

1. **Identify your outbound IPs** — In Azure App Service, go to **Properties** > **Outbound IP Addresses** (or use the **Additional Outbound IP Addresses** for all possible IPs)
2. **Configure source system allowlisting** — Add these IPs to the source system's Connected App or firewall rules
3. **Re-validate after infrastructure changes** — Outbound IPs may change if you move to a different App Service Plan or region

> ⚠️ **IP allowlisting is a defense-in-depth measure**, not a primary authentication mechanism. It blocks API calls from unauthorized networks but does not replace proper authentication and authorization controls.

### 12.9 Security Review & Admin Consent Preparation

Enterprise security teams typically require a formal **permission justification package** before granting admin consent for Microsoft Graph application permissions. Prepare the following documentation:

#### Permission Justification Package Checklist
...
| **Architecture Diagram** | Show the end-to-end data flow | Source system → Connector → Microsoft Graph → M365 Search/Copilot |
| **API Operations Inventory** | Enumerate every API call | HTTP method, endpoint, purpose, frequency, whether it writes data (see Section 12.4) |
| **Blast Radius Analysis** | Assess credential compromise impact | Risk level, impact, mitigations for each compromise scenario (see Section 12.6) |
| **Security Controls Summary** | Document defense-in-depth layers | Each layer, what it restricts, implementation status (see Section 12.5) |
| **Source System Hardening** | Detail source-side security posture | Integration user, read-only profile, FLS, connected app policies (see Section 12.3) |
| **Secret Management Plan** | Explain credential storage and rotation | Key Vault, Managed Identity, RBAC, secret rotation schedule |
| **Deployment Architecture** | Describe hosting and scheduling | Compute host, crawl schedule, auto-recovery strategy (see Section 12.7) |

#### Key Messages for Security Reviewers

When presenting to your security team, emphasize:

1. **Minimal permissions** — `.OwnedBy` is the least-privileged variant available for Graph Connectors; it cannot access any other Microsoft 365 data
2. **Read-only source access** — The connector performs zero write operations to the source system (when properly hardened)
3. **Zero credentials in code** — All secrets are stored in Azure Key Vault and accessed via Managed Identity
...
- [ ] Field-Level Security configured to block sensitive fields on source system
...
- [ ] API operations inventory documented (every HTTP method, endpoint, and purpose)
